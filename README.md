...
既存データを残したまま、目標件数まで少しずつ追加:

```bash
node generate.js --target-count 365 --batch-size 30
```

この例では、現在 180 件なら 30 件だけ追加して 210 件になり、次回も同じコマンドで 240 件、270 件と段階的に増やせます。
...
- 作品数はまず `180 → 365 → 500 → 1000` の順で段階的に増やす
- `1000` 件までは現在の単一 `translations.json` 構成を維持する
- `1001` 件以上に拡張したくなった場合は、初回ロード負荷を避けるため **JSON 分割を検討してから増やす**
- その場合は、年単位・チャンク単位・`date -> objectId` と作品本体の分離、のいずれかを候補にする
- 365 件への次回拡張は、一括ではなく `30件ずつ` を基本にする
- 推奨手順は `180 → 210 → 240 → 270 ... → 365` です
- 増分追加では `node generate.js --target-count 365 --batch-size 30` を毎回同じように実行する
